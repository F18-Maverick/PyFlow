/*
 * pf_rsa.c - RSA-OAEP (SHA-256) key management and encryption.
 */
#include "pf_rsa.h"
...
pf_err_t pf_rsa_keygen(int bits, pf_rsa_key_t **out_key) {
    if (out_key == NULL) {
        return PF_ERR_INVALID_ARG;
    }
    if (bits < (int)PF_RSA_MIN_BITS || bits > (int)PF_RSA_MAX_BITS) {
        return PF_ERR_INVALID_ARG;
    }
    ...
    pf_rsa_key_t *key = (pf_rsa_key_t *)calloc(1, sizeof(*key));
    if (key == NULL) {
        EVP_PKEY_free(pkey);
        return PF_ERR_NOMEM;
    }
...
static pf_err_t read_pem_key(const char *path, const char *passphrase, int want_private,
...
pf_err_t pf_rsa_read_pub(const char *path, pf_rsa_key_t **out_key) {
    return read_pem_key(path, NULL, 0, out_key);
}

pf_err_t pf_rsa_read_priv(const char *path, const char *passphrase, pf_rsa_key_t **out_key) {
    return read_pem_key(path, passphrase, 1, out_key);
}

pf_err_t pf_rsa_write_pub(const pf_rsa_key_t *key, const char *path) {
...
pf_err_t pf_rsa_write_priv(const pf_rsa_key_t *key, const char *path,
...
size_t pf_rsa_ciphertext_len(const pf_rsa_key_t *key) {
...
size_t pf_rsa_max_plaintext_len(const pf_rsa_key_t *key) {
    size_t k = pf_rsa_ciphertext_len(key);
    if (k == 0) {
        return 0;
    }
...
pf_err_t pf_rsa_encrypt(const pf_rsa_key_t *key, const uint8_t *in, size_t in_len,
                        uint8_t *out, size_t *out_len) {
    if (key == NULL || key->pkey == NULL || in == NULL || in_len == 0 || out_len == NULL) {
        return PF_ERR_INVALID_ARG;
    }
    if (in_len > pf_rsa_max_plaintext_len(key)) {
        return PF_ERR_INVALID_ARG;
    }
    ...
    if (*out_len < need) {
        *out_len = need;
        EVP_PKEY_CTX_free(ctx);
        return PF_ERR_BUFFER_TOO_SMALL;
    }
    size_t actual = need;
...
pf_err_t pf_rsa_decrypt(const pf_rsa_key_t *key, const uint8_t *in, size_t in_len,
                        uint8_t *out, size_t *out_len) {
    if (key == NULL || key->pkey == NULL || in == NULL || in_len == 0 || out_len == NULL) {
        return PF_ERR_INVALID_ARG;
    }
...
void pf_rsa_key_free(pf_rsa_key_t *key) {
    if (key == NULL) {
        return;
    }
